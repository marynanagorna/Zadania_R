...
# Plik powinien znajdować się w tym samym folderze co ten skrypt .R
############################################################
...
# Instalacja pakietów - uruchom tylko wtedy, gdy pakiety nie są zainstalowane
# install.packages("tm")
...
# W notatkach było stemCompletion, ale tutaj dla sprawnego działania projektu używamy korpusu po czyszczeniu.
corpus_final <- corpus
...
# Term-Document Matrix: tokeny w wierszach, dokumenty w kolumnach
tdm <- TermDocumentMatrix(corpus_final)

# Document-Term Matrix: dokumenty w wierszach, tokeny w kolumnach
dtm <- DocumentTermMatrix(corpus_final)

# Usunięcie bardzo rzadkich słów, aby skrypt działał sprawnie
# 0.99 oznacza, że zostawiamy słowa występujące w większej liczbie dokumentów.
...
# relationship = "many-to-many" usuwa ostrzeżenie z dplyr i jest zgodne z tym typem danych
...
# Słownik NRC: emotions + positive/negative
...
# Usunięcie pustych dokumentów, bo LDA nie działa na pustych wierszach
...
# Ograniczenie liczby dokumentów, żeby działało szybko w RStudio
# Można zwiększyć 1000 na 2000 lub 5000, jeśli komputer jest mocny.
set.seed(123)
...
# Zgodnie z notatkami ustawiamy k ręcznie i analizujemy wyniki.
set.seed(123)
k <- 4
